...
export const LiveCommandMap: React.FC<LiveCommandMapProps> = ({
  ...
  return (
    <div className="relative w-full h-full min-h-[340px] sm:min-h-[420px] rounded-2xl overflow-hidden border border-white/10 shadow-2xl isolate z-0">
      <MapContainer
        center={center}
        zoom={zoom}
        minZoom={3}
        ...
        style={{ minHeight: '340px', background: '#070A11' }}
      >
      ...
      <div className="absolute top-3 right-3 z-10 flex flex-wrap items-center gap-1 p-1 rounded-xl bg-obsidian-850/95 backdrop-blur-md border border-white/10 shadow-xl max-w-[calc(100%-4rem)] justify-end">
        <div className="hidden sm:flex items-center gap-1 px-1.5 text-gray-400">
        ...
        {(['matrix', 'tactical', 'satellite', 'osm'] as const).map((mode) => (
          <button
            ...
            className={`px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg text-[10px] sm:text-[11px] font-mono font-medium transition-all ${
              tileMode === mode
      ...
      {/* Live Map Legend */}
      <div className="absolute bottom-3 left-3 z-10 bg-obsidian-850/90 backdrop-blur-md border border-white/10 p-2 sm:p-3 rounded-xl shadow-xl flex flex-wrap items-center gap-2 sm:gap-4 text-[10px] sm:text-xs max-w-[calc(100%-1.5rem)]">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-gray-300 font-medium">Hazard</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-cyan-500"></span>
